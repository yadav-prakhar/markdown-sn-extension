...
`;
  ...
  const BLOCKQUOTE_CSS = `<style type="text/css">
.blockquote { background-color: #f6f8fa; border-left: 4px solid #57606a; padding: 8px 12px; display: block; margin: 8px 0; font-size: 110%; color: #24292f; }
</style>
`;
...
`;
  ...
  function formatBlockquote(lines, alertType, alertConfigs) {
    ...
    if (alertType && alertConfigs[alertType]) {
      ...
      return `<p class="${alertType}">${emoji} <strong>${displayName}:</strong> ${content}</p>`;
    }
    // NOTE: We intentionally keep semantic <blockquote> for accessibility
    // and HTML correctness. Styling is applied via class-based CSS injection.
  ...
  function wrapWithCodeTags(text, alertConfigs) {
    ...
    if (/\bclass=["'][^"']*\bblockquote\b/.test(text)) {
      css += BLOCKQUOTE_CSS;
    }
