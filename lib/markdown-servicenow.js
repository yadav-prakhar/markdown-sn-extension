...
// Based on markdown-servicenow npm package v1.0.4
...
`;
  ...
  const ALERT_CSS_RULES = {
    status: '.status { color: #24292f; background-color: #f1f3f5; padding: 10px 14px; border-left: 4px solid #57606a; display: block; margin: 10px 0; font-size: 110%; }',
    important: '.important { color: #5a2ea6; background-color: #f4ecff; padding: 8px 12px; border-left: 4px solid #8250df; display: block; margin: 8px 0; font-size: 110%; }',
    success: '.success { color: #055f5b; background-color: #e0f2f1; padding: 8px 12px; border-left: 4px solid #2aa198; display: block; margin: 8px 0; font-size: 110%; }',
    note: '.note { color: #325d7a; background-color: #eaf2f8; padding: 8px 12px; border-left: 4px solid #5b8def; display: block; margin: 8px 0; font-size: 110%; }',
    tip: '.tip { color: #0f766e; background-color: #e6fffb; padding: 8px 12px; border-left: 4px solid #14b8a6; display: block; margin: 8px 0; font-size: 110%; }',
    attention: '.attention { color: #6b4f1d; background-color: #f6efe3; padding: 8px 12px; border-left: 4px solid #b0882c; display: block; margin: 8px 0; font-size: 110%; }',
    warning: '.warning { color: #7c5e10; background-color: #faf3d1; padding: 8px 12px; border-left: 4px solid #d4a72c; display: block; margin: 8px 0; font-size: 110%; }',
    caution: '.caution { color: #7a1f3d; background-color: #fdecef; padding: 8px 12px; border-left: 4px solid #c9375e; display: block; margin: 8px 0; font-size: 110%; }',
    blocker: '.blocker { color: #2d1b3d; background-color: #ede7f6; padding: 8px 12px; border-left: 4px solid #6f42c1; display: block; margin: 8px 0; font-size: 110%; }',
    question: '.question { color: #4f4a2f; background-color: #f6f4ea; padding: 8px 12px; border-left: 4px solid #d4cb94; display: block; margin: 8px 0; font-size: 110%; }'
  };
...
`;
    ...
    for (const line of lines) {
      if (line.startsWith('>')) {
        ...
        if (blockquoteLines.length === 0 && content.startsWith('[!')) {
          const match = content.match(/^\[!(STATUS|IMPORTANT|SUCCESS|NOTE|TIP|ATTENTION|WARNING|CAUTION|BLOCKER|QUESTION)\]/);
          if (match) {
  ...
  function wrapWithCodeTags(text) {
    ...
    const alertTypes = ['status', 'important', 'success', 'note', 'tip', 'attention', 'warning', 'caution', 'blocker', 'question'];
    const usedAlerts = alertTypes.filter(type => text.includes(`class="${type}"`));
    if (usedAlerts.length > 0) {
      const alertCss = usedAlerts.map(type => ALERT_CSS_RULES[type]).join('\n');
